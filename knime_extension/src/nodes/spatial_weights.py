...
@knext.node(name="Spatial Weights", node_type=knext.NodeType.MANIPULATOR, icon_path="icons/icon.png", category=category)
...
# FIXME:
    ...
    category = knext.StringParameter("Weights category", "The default value is ‘Queen’ which will construct a queen contiguity weights matrix. Queen weights is more robust and more suitable for areal unit data. The queen criterion is somewhat more encompassing and defines neighbors as spatial units sharing a common edge or a common vertex. The rook criterion defines neighbors by the existence of a common edge between two spatial units. Therefore, the number of neighbors according to the queen criterion will always be at least as large as for the rook criterion. When choosing k-nearest, select the nearest number 'Nearest k' in the following options. When selecting Binary Distance Band, please select the distance threshold 'Threshold' in the following options. When selecting Inverse Distance, please select the distance threshold 'Threshold' and the corresponding power 'Power' in the following options. When 'Your own' is selected, please enter the path of the spatial weights matrix in CSV format in the following options. More details about spatial weights, please see the GeoDa center website: https://geodacenter.github.io/documentation.html", "Queen",enum=["Queen", "Rook", "Inverse Distance", "Binary Distance Band","K nearest","Lattice","Kernel","Your own"])
    order =  knext.IntParameter("Order", "The order of the weight matrix is 1 by default. Users can change the order of the weights, higher order weights will treat further units as neighbors.", 1)

    Nearest_k =  knext.IntParameter("Nearest k", "K-nearest are often used for point data. k is the number of the nearest neighbor", 4)

    Threshold = knext.IntParameter("Threshold", "Distance band weights are often used for point data. The weights within the threshold are 1 and otherwise 0. Inverse distance weights are often used for point data. The weights within the threshold are distance^-power, and otherwise 0. The distance is Euclidean distance.", 1)
    Power = knext.IntParameter("Power", "Distance band weights are often used for point data. The weights within the threshold are 1 and otherwise 0. Inverse distance weights are often used for point data. The weights within the threshold are distance^-power, and otherwise 0. The distance is Euclidean distance.", 1)
    Rows = knext.IntParameter("Rows", "Please choose your rows and colunns of your lattice.", 5)
    Columns = knext.IntParameter("Columns", "Please choose your rows and columns of your lattice.", 5)
    Your_own_matrix_local_path =  knext.StringParameter("Your own matrix local path", "Please enter the path of the spatial weights matrix in CSV format in the following options. The weights matrix must be in matrix format and in the order of the samples.", "")
    # FIXME:
    Kernel_type = knext.StringParameter("Kernel type", " ", "triangular",enum=["triangular","uniform","quadratic","quartic","gaussian"])
    Kernel_K = knext.IntParameter("Kernel K", " ", 12)
    # FIXME:
    Kernel_bandwidth = knext.StringParameter("Kernel bandwidth", " ", "Fixed",enum=["Fixed","Adaptive"])
    ...
    def execute(self, exec_context:knext.ExecutionContext, input_1):
        ...
        if self.category == 'Inverse Distance':
            w=libpysal.weights.DistanceBand.from_dataframe(gdf,self.Threshold,alpha=-1*self.order,binary=False)
            wname='Inverse Distance'
            w.transform='r'
        if self.category == 'Binary Distance Band':
            w=libpysal.weights.DistanceBand.from_dataframe(gdf,self.Threshold,binary=True)
        ...
        if self.category== 'Kernel':
            ...
            if self.Kernel_bandwidth == 'Fixed':
                bd=True
            w=libpysal.weights.Kernel.from_dataframe(gdf,fixed=bd,k=self.Kernel_K,function=self.Kernel_type)
        ...
        exec_context.set_progress(0.1, "Constructs a contiguity spatial weights matrix done")
